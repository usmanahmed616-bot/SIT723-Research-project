...
class CDS:
    ...
    def get_merge(self,start_index, end_index, data, nb_jobs):
    ...
    def cdsAlgo(self,data, nb_machines, nb_jobs):
        c_max_best = float("inf")
        for k in range(1, nb_machines):
            p1_prime = self.get_merge(0, k, data, nb_jobs)
            p2_prime = self.get_merge(nb_machines - k, nb_machines, data, nb_jobs)
    ...
    def test(self):
        nbm, nbj, p_ij = commonFunction.read_from_file("cdsTest.txt")
        seq, cmax = self.cdsAlgo(p_ij, nbm, nbj)
